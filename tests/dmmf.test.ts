...
describe("getMainInput", () => {
    ...
    test("should priorize not scalar", () => {
        ...
        // prisma.user.findFirst({ where: { OR: [{ name: "Emanuel" }] } })
        expect(getMainInput().priorizeNotScalar(list)?.namespace).toBe(
            "prisma"
        );
        const list2: DMMF.InputTypeRef[] = [
            ...
            { type: "DateTime", location: "scalar", isList: false },
            { type: "Null", location: "scalar", isList: false },
        ];
        // prisma.user.findFirst({ where: { OR: [{ name: "Emanuel" }] } })
        expect(getMainInput().priorizeNotScalar(list2)?.namespace).toBe(
            "prisma"
        );
    });
    ...
    test("should priorize WhereInput over RelationFilter", () => {
        ...
        expect(getMainInput().priorizeWhereInput(list)?.type).toBe(
            "ProfileWhereInput"
        );
    ...
    test('in update, should priorize "set" instead of directly set', () => {
        ...
        expect(getMainInput().run(list)?.type).toBe(
            "EnumPAYMENT_METHODFieldUpdateOperationsInput"
        );
    });
});
